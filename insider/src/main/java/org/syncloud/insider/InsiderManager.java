...
public class InsiderManager {
    ...
    public static Result<String> addService(
        ...
        Result<String> result = execute1(
                device,
                asList(format(
                        "%s add_service %s %s %s %s %s",
                        INSIDER_BIN, name, protocol, type, port, url)));

        return flatten(result.map(new Result.Function<String, Result<String>>() {
            @Override
    ...
    public static Result<Endpoint> serviceInfo(Device device, String name) {
        Result<String> result = execute1(
                device, asList(format("%s service_info %s", INSIDER_BIN, name)));
        return result.map(new Result.Function<String, Endpoint>() {
            @Override
            public Endpoint apply(String input) throws Exception {return JSON.readValue(input, Endpoint.class);
            }
        });
    }
    ...
    public static Result<String> fullName(Device device) {
        Result<String> result = execute1(device, asList(INSIDER_BIN + " full_name"));
        return result
                .map(new Result.Function<String, String>() {
                    @Override
                    public String apply(String input) throws Exception {return JSON.readValue(input, InsiderResult.class).getData();
                    }
                });
